 ...
 *
 * Il porte aussi le premier filtre de l'espace d'administration : sans
 * session, on n'affiche pas la console — même vide. C'est une vérification
 * OPTIMISTE, au sens de la documentation Next : elle évite d'envoyer la
 * console à un visiteur anonyme, elle ne remplace ni la RLS (qui décide de
 * chaque ligne), ni les routes /api/admin/* (qui relisent le rôle et les
 * permissions de l'appelant en base).
 */
...
export async function proxy(request: NextRequest) {
  ...
  if (!user && chemin.startsWith("/espace-admin") && chemin !== CONNEXION_ADMIN) {
    const cible = request.nextUrl.clone();
    cible.pathname = CONNEXION_ADMIN;
