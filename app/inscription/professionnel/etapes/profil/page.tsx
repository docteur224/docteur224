 ...
 * présentation, langues, soins, genre. Tout est écrit dans `medecins`,
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data: m } = await supabase
        .from("medecins")
        .select("specialite_id, tarif_consultation, annees_experience, presentation, langues, soins_et_actes, genre")
        .eq("id", auth.user.id)
        .maybeSingle();
      if (!actif || !m) return;
